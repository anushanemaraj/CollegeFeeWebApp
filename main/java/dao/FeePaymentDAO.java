...
public class FeePaymentDAO {
    ...
    private static final String SELECT_TOTAL_COLLECTION_BY_DATE_RANGE = "SELECT SUM(Amount) AS TotalAmount FROM FeePayments WHERE PaymentDate BETWEEN ? AND ?";
    private static final String SELECT_UNPAID_PAYMENTS_BY_DATE_RANGE = "SELECT * FROM FeePayments WHERE Status = 'Unpaid' AND PaymentDate BETWEEN ? AND ?";
    ...
    public void insertPayment(FeePayment fee) throws SQLException {
        try (Connection connection = getConnection();
            ...
            preparedStatement.setTimestamp(3, new java.sql.Timestamp(fee.getPaymentDate().getTime())); // use Timestamp here
    ...
    public FeePayment selectPayment(int paymentID) {
        ...
        try (Connection connection = getConnection();
            ...
            if (rs.next()) {
                ...
                payment.setPaymentDate(rs.getTimestamp("PaymentDate"));  // use getTimestamp()
    ...
    public List<FeePayment> selectAllPayments() {
        ...
        try (Connection connection = getConnection();
            ...
            while (rs.next()) {
                ...
                payment.setStudentID(rs.getString("StudentID"));
                payment.setStudentName(rs.getString("StudentName"));
                payment.setPaymentDate(rs.getTimestamp("PaymentDate"));  // getTimestamp
                payment.setAmount(rs.getDouble("Amount"));
                payment.setStatus(rs.getString("Status"));
    ...
    public boolean updatePayment(FeePayment fee) throws SQLException {
        ...
        try (Connection connection = getConnection();
            ...
            preparedStatement.setString(1, fee.getStudentID());
            preparedStatement.setString(2, fee.getStudentName());
            preparedStatement.setTimestamp(3, new java.sql.Timestamp(fee.getPaymentDate().getTime())); // Timestamp here
            preparedStatement.setDouble(4, fee.getAmount());
            preparedStatement.setString(5, fee.getStatus());
    ...
    public List<FeePayment> selectPaymentsByDateRange(Date fromDate, Date toDate) {
        List<FeePayment> payments = new ArrayList<>();
        System.out.println("Selecting payments by date range: from " + fromDate + " to " + toDate);
        try (Connection connection = getConnection();
    ...
    // Get total collection amount by date range
    public double getTotalCollectionByDateRange(Date fromDate, Date toDate) {
        double totalAmount = 0.0;
        System.out.println("Getting total collection from " + fromDate + " to " + toDate);
        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_TOTAL_COLLECTION_BY_DATE_RANGE)) {
        ...
        System.out.println("Total collection: " + totalAmount);
        return totalAmount;
    }

    // Select unpaid payments by date range
    public List<FeePayment> selectUnpaidPaymentsByDateRange(Date fromDate, Date toDate) {
        List<FeePayment> payments = new ArrayList<>();
        System.out.println("Selecting unpaid payments from " + fromDate + " to " + toDate);
        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_UNPAID_PAYMENTS_BY_DATE_RANGE)) {
        ...
        System.out.println("Unpaid payments found: " + payments.size());
        return payments;
    }
}
